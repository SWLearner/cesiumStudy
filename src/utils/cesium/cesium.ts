...
export function draw(viewer: Cesium.Viewer, type: string) {
  ...
  if (type == "LineString") {
    ...
    handler.setInputAction(function (
      ...
      let ray: Cesium.Ray | undefined = viewer.camera.getPickRay(
        click.position
      );
      myPosition = viewer.scene.globe.pick(ray!, viewer.scene);
      //加入第一点
      ...
      if (tempLength > 1) {
        drawPolyline(viewer, [
          tempPoints[tempLength - 2],
          tempPoints[tempLength - 1],
        ]);
...
export function measurePolyline(viewer: Cesium.Viewer) {
  ...
  handler.setInputAction(function (
    ...
    //空间坐标
    let myPosition: Cesium.Cartesian3 = viewer.scene.pickPosition(
      click.position
    );
    if (!myPosition) {
      return false;
    ...
    if (points.length == 0) {
      ...
      handler.setInputAction(
        (moveEvent: Cesium.ScreenSpaceEventHandler.MotionEvent) => {
          ...
          let movePosition: Cesium.Cartesian3 = viewer.scene.pickPosition(
          ...
          if (points.length == 2) {
            ...
            let centerPoint: Cesium.Cartesian3 = Cesium.Cartesian3.midpoint(
              points[0],
              points[1],
...
export function measurePolygon(viewer: Cesium.Viewer) {
  ...
  handler.setInputAction(
    (moveEvent: Cesium.ScreenSpaceEventHandler.MotionEvent) => {
      var movePosition = viewer.scene.globe.pick(
      ...
      if (positions.length >= 3) {
        ...
        var centerPoint: Cesium.Cartesian3 =
  ...
  handler.setInputAction(
    (clickEvent: Cesium.ScreenSpaceEventHandler.PositionedEvent) => {
      var clickPosition = viewer.scene.globe.pick(
        viewer.camera.getPickRay(clickEvent.position)!,
        viewer.scene
      );
      if (!clickPosition) {
 ...
 * @returns 返回两点的长度
 */
function getSpaceDistance(
  firstPoint: Cesium.Cartesian3,
  secondPoint: Cesium.Cartesian3
) {
...
//entity
export function createEntityPolygon(
  viewer: Cesium.Viewer,
...
export function addPoint(viewer: Cesium.Viewer) {
  const point = new Cesium.Entity({
    id: "point", //唯一标识符
    name: "点",
    show: true, //控制是否显示
    position: Cesium.Cartesian3.fromDegrees(118, 32, 40.0), //位置，需要将坐标转为笛卡尔直角坐标
    point: {
  ...
  viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(118, 32, 40.0),
  });
}
export function addLine(viewer: Cesium.Viewer) {
  ...
  viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(119, 32, 400.0),
  });
}